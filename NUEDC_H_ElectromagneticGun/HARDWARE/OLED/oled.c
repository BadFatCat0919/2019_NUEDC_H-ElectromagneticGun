...
void OLED_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin		= GPIO_Pin_2;             //设置初始化引脚编号
  GPIO_InitStructure.GPIO_Mode	= GPIO_Mode_Out_PP;       //设置IO模式为推挽输出
  GPIO_Init(GPIOB, &GPIO_InitStructure);							    //读取初始化结构体的值
  GPIO_InitStructure.GPIO_Pin		= GPIO_Pin_7 |GPIO_Pin_8| //设置初始化引脚编号
                                  GPIO_Pin_10|GPIO_Pin_12;
  GPIO_Init(GPIOE, &GPIO_InitStructure);							    //读取初始化结构体的值
  
  GPIO_SetBits(GPIOB,GPIO_Pin_2);                         //IO初始化完成后设置默认电平
